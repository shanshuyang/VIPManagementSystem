...
{
    /// <summary>
    /// 说明：JuniorAdministrator类是拥有最低权限的管理员。
    /// </summary>
    public class JuniorAdministrator : Administrator
    {
        ...
        /// <summary>
        /// 说明：JuniorAdministrator类的构造函数，权限等级最低，默认只拥有查询会员的权限。
        /// </summary>
        /// <param name="name">管理员的用户名</param>
        public JuniorAdministrator(string name)
        {
            Name = name;
            Permission = PermissionLevel.Junior;
            ManageAdministratorPermission = false;
            AddVIPPermission = false;
            QueryVIPPermission = true;
            RechargeMoneyPermission = false;
            SpendMoneyPermission = false;
        }
    }
}
